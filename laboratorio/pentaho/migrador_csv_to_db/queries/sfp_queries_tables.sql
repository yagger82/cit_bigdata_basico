/* UNIVERSIDAD NACIONAL DE ASUNCION
 * Facultad Politécnica - Proyecto Centro de Innovación TIC - Curso Básico de Introducción a Big Data
 * 
 * Descripción: Consultas de tablas de SFP.
 *
 * @autor: Prof. Richar D. Jiménez-R. <rjimenez@pol.una.py>
 * @creacion: Setiembre 10, 2024
 * @ultima_modificacion: Setiembre 10, 2024
 * @version: 1.0.0
 */

--PARAMETRICAS
select * from sfp.periodo;
select count(*) from sfp.periodo;


select * from sfp.tipo_discapacidad;
select count(*) from sfp.tipo_discapacidad;


select * from sfp.estado;
select count(*) from sfp.estado;


select * from sfp.fuente_financiamiento;
select count(*) from sfp.fuente_financiamiento;


select * from sfp.sexo;
select count(*) from sfp.sexo;


select * from sfp.nacionalidad;
select count(*) from sfp.nacionalidad;


select * from sfp.control_financiero;
select count(*) from sfp.control_financiero;


select * from sfp.grupo_gasto;
select count(*) from sfp.grupo_gasto;


select * from sfp.subgrupo_gasto;
select count(*) from sfp.subgrupo_gasto;


select * from sfp.objeto_gasto;
select count(*) from sfp.objeto_gasto;


--NIVEL
select * from nivel;
select count(*) from nivel;

--ENTIDAD
select * from entidad;
select count(*) from entidad;

--OEE
select * from oee;
select count(*) from oee;

--SEXO
select * from sexo;
select count(*) from sexo;

--TIPO DISCAPACIDAD
select * from tipo_discapacidad;
select count(*) from tipo_discapacidad;

--NACIONALIDAD
select * from nacionalidad;
select count(*) from nacionalidad;

--PERSONA
select * from persona;
select count(*) from persona;
